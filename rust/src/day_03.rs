...
pub fn solve(input: &String) -> (Option<String>, Option<String>) {
    ...
    let most_common_bits: Vec<_> = (0..n_bits)
        .map(|i| get_most_common_bit(&bitstrings, i))
        .collect();
...
fn reduce_candidates(bitstrings: Vec<String>, index: usize, bit_criteria: fn(u32) -> char) -> Vec<String> {
    if bitstrings.len() == 1 {
        return bitstrings;
    }
    let mcb = get_most_common_bit(&bitstrings, index);
...
fn get_most_common_bit(bitstrings: &Vec<String>, index: usize) -> u32 {
...
#[allow(dead_code)]
pub fn solve_fast(input: &String) -> (Option<String>, Option<String>) {
    ...
    for i in (0..n_bits).rev() {
        let mcb = get_most_common_bit_2(&numbers, i);
        gamma = 2 * gamma + mcb;
        epsilon = 2 * epsilon + mcb;
        oxygen = reduce_candidates_2(oxygen, i, |mcb| if mcb == 0 { 0 } else { 1 });
        co2 = reduce_candidates_2(co2, i, |mcb| if mcb == 0 { 1 } else { 0 });
    }
...
fn get_most_common_bit_2(numbers: &Vec<u32>, index: usize) -> u32 {
...
fn reduce_candidates_2(numbers: Vec<u32>, index: usize, bit_criteria: fn(u32) -> u32) -> Vec<u32> {
    if numbers.len() == 1 {
        return numbers;
    }
    let mcb = get_most_common_bit_2(&numbers, index);
    let target = bit_criteria(mcb);
    numbers
        .into_iter()
